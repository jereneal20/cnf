...
def convertToCNF(formula):
	equForm = equivalanceFree(formula)
	impFreeForm = implicationFree(equForm)
	negFreeForm = negationNormalForm(impFreeForm)
	conjNormForm = conjunctiveNormalForm(negFreeForm)
	return conjNormForm